...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    kodaksmarthome_cams = hass.data[DATA_KODAKSMARTHOME_CAMS]

    cams = []
    cam_offline = []
    for camera in kodaksmarthome_cams.list_devices:
        if camera["is_online"]:
            cams.append(
                KodakSmartHomeCam(
                    hass,
                    camera,
                    kodaksmarthome_cams.get_motion_events(
    ...
    if cam_offline:
        ...
        err_msg = """Cameras are offline""" """ following cameras: {}.""".format(
            cameras
        )
